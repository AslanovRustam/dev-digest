...
describe('AI contracts parse fixtures', () => {
  ...
  it('RunStats tolerates pre-L01 traces without cost_usd (unknown, not $0)', () => {
    const stats = RunStats.parse({ duration_ms: 1, tokens_in: 1, tokens_out: 1, findings: 0, grounding: '0/0 passed' });
    expect(stats.cost_usd).toBeUndefined();
  });
});
